...
function convertWikiListsToMarkdown(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (listMatch) {
            ...
            // Bold/italic: ** followed immediately by non-space (e.g., **text**)
            // List item: ** followed by space or nothing (e.g., ** item)
            if (/^\*{2,}/.test(markers) && afterMarkers && !/^\s/.test(afterMarkers)) {
                result.push(line);
                continue;
            }
