...
function convertWikiListsToMarkdown(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Use alternation to match ONLY pure sequences: #+  OR  *+  (not mixed like #**)
        ...
        if (listMatch) {
            ...
            // Bold text has opening and closing markers: **text**
            // List items don't have closing markers: **List of things
            // Check for closing markers to distinguish
            const hasClosingMarker = afterMarkers.includes('**') || afterMarkers.includes('***');
            if (/^\*{2,}/.test(markers) && hasClosingMarker) {
                // This is bold markdown, not a list
                result.push(line);
                continue;
            }

            // Extract content (trim leading space from afterMarkers)
            const content = afterMarkers.trimStart();
