 ...
 */
function convertTemplate(node: Token, context: ConversionContext = {}): string {
  ...
  if (nameLower === 'main') {
    if (args.length > 0) {
      ...
      const availablePagesMap = context.availablePagesMap || new Map();
      const linkTarget = getLinkTarget(page, context.currentPage || '', availablePagesMap);
...
function convertNode(node: AstNodes, context: ConversionContext = {}): string {
  ...
  switch (type) {
    ...
    case 'template': {
      return convertTemplate(node, context);
    }
...
function convertXmlToMarkdown(
  ...
  const context = { currentPage: pageName, availablePagesMap: pagesMap };
