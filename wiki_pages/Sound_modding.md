...
# Download this [FMOD project template](https://github.com/Agamidae/1.10-FMOD-CK3-template) from Github.
# Unzip it, open '1.10 fmod ck3 template.fspro'.
#* If FMOD asks to recover the file, agree. Re-save it as your own project, so you can always reuse the template.
# Add your sounds to the Assets tab.
#* It accepts wav, mp3, ogg, aiff, wma and flac.
# Right-click them and create events (any type, pick 2D if unsure).
#* You can rename events by double-clicking and change their volume at the bottom of the screen.
# Go to Events tab, right-click your events and assign them to the Bank bank.
# Go to Banks tab and rename the bank to something unique to avoid conflicts with other mods or game files.
#* IMPORTANT! The name of your bank should precede "Master Bank". Try A-L range. See below for details.
# Go to Window > Mixer Routing (Ctrl+5).
# Right-click your events and assign them to appropriate VCAs, these are game's volume controls. Without it, our sounds would blast players at full volume.
# Save and build the project from File > Build (F7). It will create bank files in the project's ``Build/banks`` folder.
# Copy the banks to your mod's ``/sound/banks`` folder. You'll have two files, copy both.
#* Check for typos! That's singular ``sound`` and plural ``banks``.
# For the future, you can tell FMOD to build right to the mod folder, in Edit -> Preferences -> Build.
#* if you change the name of your bank, you'll need to remove the old bank files manually
# Launch the game with your mod and test with console command ``Audio.PlayEvent event:/myevent`` 
...
# Create your project and add new events, following the steps from above.
# Go to Window > Mixer Routing, VCAs tab. Create a new VCA and name it one of these names, depending on what your sounds are:
#* Ambience, Music, Sound Effects, UI
# In the Routing tab, expand the Master Bus and assign your sounds to the VCA. This will allow players to adjust their volume.
#* Note, you may need to assign one event to multiple VCAs. UI sounds are also affected by Sound Effects volume, for example.
# Save your project (don't put it in the mod folder yet).
# Go to File > Export GUIDs. This will create a text file in your project's Build/ folder.
# Go to that folder and open GUIDs.txt.
# Find the line ending with ``bus:/`` and another with the name of your VCA and copy the ids to another file somewhere. They might look like this:
#* ``{767eec3a-3ca8-4ae8-8827-376bf7db4d8f} bus:/``
#* ``{72d40a2a-0111-4078-8ba7-e84d415b91a2} vca:/UI``
# In CK3 folder, open game/sound/GUIDs.txt and find its ``bus:/`` and vca lines, copy their ids as well. The VCAs are at the bottom. E.g.: 
#* ``{cb930c67-0464-4d7f-957a-a78b08fc39de} bus:/``
#* ``{f8bd5083-a8cc-412b-ada4-cdc08a33ce75} vca:/UI``
# In your FMOD project folder, search through all the files in the Metadata folder and replace your ids, inside {}, with the ones from the game, for both the bus and vca.
...
# Go back to FMOD and restart it, reopen your project.
# Select File > Build. This will create our bank files that will be used by the game.
# Copy all the bank files created and put them into your mod, in sound/banks folder.
...
# Add Parameter Sheet by clicking the +  view next to Timeline and clicking 'Add Parameter Sheet' > 'New Parameter'.
# Select 'Built-in: Distance'.
# Add a Multiband EQ on the instrument.
# Right click Freq. (A) and modulate it with distance.
# Draw your curve and test it out live by changing the distance parameter.
...
# Select the Event that needs the parameter and enter the Event Editor.
# Press the '+' next to 'Action' or 'Timeline' then press 'Add Parameter Sheet' then 'New Parameter'.
# Name your parameter. Use 'User: Discrete' Parameter type, if you want the values in integers otherwise use 'User: Continuous'. Press OK.
# The parameter can now be used to automate Volume, Effects, etc.
# Right-click the desired value(s) (Volume, panning, reverb length, etc.) and press 'Add Automation'.
# Press on 'Add Curve' then 'Browse > ParameterName'.
# Draw a curve by double-clicking in the graph to make points.
# The sound can be tested live by changing the parameter in the 'Parameters' pane on the right.
