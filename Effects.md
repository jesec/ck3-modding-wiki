...
The full list of available code effects can be found in [effects.log](Effects_list.md).


- [Effect blocks](#effect-blocks)
- [Effect syntax](#effect-syntax)
...
| else_if | Executes enclosed effects if limit criteria of preceding 'if' or 'else_if' is not met, and its own limit is met | <code style="white-space: pre">if = { limit = { &lt;triggers&gt; } &lt;effects&gt; }<br>else_if = { limit = { &lt;triggers&gt; } &lt;effects&gt; }</code> | none |  |
| else | Executes enclosed effects if limit criteria of preceding 'if' or 'else_if' is not met | <code style="white-space: pre">if = { limit = { &lt;triggers&gt; } &lt;effects&gt; }<br>else = { &lt;effects&gt; }</code> | none |  |
| while | Repeats enclosed effects while limit criteria are met or until set iteration count is reached | <code style="white-space: pre">while = {<br>    limit = { &lt;triggers&gt; }<br>    &lt;effects&gt;<br>}<br>while = { count = 3 &lt;effects&gt; }<br>Default max of 1000.</code> | none |  |
| switch | Switch on a trigger for the evaluation of another trigger with an optional fallback trigger. | <code style="white-space: pre">switch = {<br>    trigger = simple_assign_trigger<br>    case_1 = { &lt;effects&gt; }<br>    case_2 = { &lt;effects&gt; }<br>    case_n = { &lt;effects&gt; }<br>    fallback = { &lt;effects&gt; }<br>}</code> | none |  |
...
Current effects can be found in an ***effects.log*** file in your local data folder's script_documentation (defaults to "%USERPROFILE%\Documents\Paradox Interactive\Crusader Kings III\**logs\**"), after you have run the ``script_docs`` [command](Console_commands.md) in the debug console.
...
| move_budget_gold | Move gold from one AI budget category to the other, will not move more than is available in the source budget or what can fit in the target budget | <code style="white-space: pre">move_budget_gold = { gold = X from = Z to = Y }<br>('budget_war_chest', 'budget_reserved', 'budget_short_term', 'budget_long_term')</code> | character |  |
...
| ordered_memory | Iterate through all memories of a character | <code style="white-space: pre">ordered_memory = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character memory |
| ordered_memory_participant | Iterate through all participating character of a memory | <code style="white-space: pre">ordered_memory_participant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character memory | character |
| ordered_powerful_vassal | Iterate through the all powerful vassals of a character | <code style="white-space: pre">ordered_powerful_vassal = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
...
| add_house_modifier | Add a modifier to a house | <code style="white-space: pre">add_house_modifier = name<br>add_house_modifier = { modifier = name days/weeks/months/years = int }<br>You can also add an optional 'desc' field. This is a dynamic description that'll be used for your timed modifier</code> | dynasty house |  |
...
| ordered_house_claimed_artifact | Iterate through all claimed artifacts of the scoped house | <code style="white-space: pre">ordered_house_claimed_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | dynasty house | artifact |
| ordered_house_member | Iterate through all house members | <code style="white-space: pre">ordered_house_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | dynasty house | character |
...
| faction_start_war | The scope faction starts the war agains their target. | <code style="white-space: pre">faction_start_war = {<br>    title = [optional]<br>}</code> | faction |  |
| ordered_faction_county_member | Iterate through all faction county members | <code style="white-space: pre">ordered_faction_county_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faction | landed title |
| ordered_faction_member | Iterate through all faction character members | <code style="white-space: pre">ordered_faction_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faction | character |
...
| ordered_war_attacker | Iterate through all attackers in the war | <code style="white-space: pre">ordered_war_attacker = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | war | character |
| ordered_war_defender | Iterate through all defenders in the war | <code style="white-space: pre">ordered_war_defender = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | war | character |
| ordered_war_participant | Iterate through all participants in the war | <code style="white-space: pre">ordered_war_participant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | war | character |
...
| ordered_activity_declined | Iterate through all characters who declined an activity invite to a specific activity | <code style="white-space: pre">ordered_activity_declined = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | activity | character |
| ordered_activity_invited | Iterate through all characters who have unanswered invites to a specific activity | <code style="white-space: pre">ordered_activity_invited = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | activity | character |
| ordered_participant | Iterate through all participants in an activity | <code style="white-space: pre">ordered_participant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | activity | character |
...
| ordered_side_commander | Iterate through all commanders (the commanders of every army on the side, not just the one leading the battle) | <code style="white-space: pre">ordered_side_commander = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | combat side | character |
| ordered_side_knight | Iterate through all knights | <code style="white-space: pre">ordered_side_knight = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | combat side | character |
...
| ordered_target_title | Iterate through all casus belli's target titles | <code style="white-space: pre">ordered_target_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | casus belli | landed title |
...
| add_loot | Adds loot to the currently scoped raiding army. | <code style="white-space: pre">Usage:<br>    add_loot = VALUE</code> | army |  |
...
| add_artifact_history | Adds a history entry to the artifact, could for example be a reforging event | <code style="white-space: pre">type = enum - history entry type<br>date = jomini date - when this historical event took place<br>actor = character - who is the actor in the event, for example who created it<br>recipient = character - who is the recipient in the event, for example who was the artifact given to<br>location = province - where the event took place</code> | artifact |  |
| add_artifact_modifier | Adds a static modifier to the given artifact | <code style="white-space: pre">add_artifact_modifier = modifier_name<br>NOTE: does not support duration!</code> | artifact |  |
| add_artifact_title_history | Adds the title history of the given title to the scoped artifacts history | <code style="white-space: pre">add_artifact_title_history = {}<br>target = title scope - landed title to take history from<br>date = game date - from which date onwards to copy historical entries</code> | artifact |  |
...
| ordered_artifact_claimant | Iterate through all characters with a claim on the scoped artifact | <code style="white-space: pre">ordered_artifact_claimant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | artifact | character |
| ordered_artifact_house_claimant | Iterate through all dynasty houses with a claim on the scoped artifact | <code style="white-space: pre">ordered_artifact_house_claimant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | artifact | dynasty house |
...
| set_coa | Sets the coat of arms of a landed title, dynasty, or house to the right hand side coat of arms or that of an object of the same type | <code style="white-space: pre">set_coa = k_england<br>set_coa = scope:new_coa</code> | landed title, dynasty, dynasty house |  |
| add_county_modifier | Add a modifier to a county | <code style="white-space: pre">add_county_modifier = name<br>add_county_modifier = { modifier = name days/weeks/months/years = int }<br>You can also add an optional 'desc' field. This is a dynamic description that'll be used for your timed modifier</code> | landed title |  |
...
| every_connected_county | Iterate through all counties connected to this one. Is based on top liege | <code style="white-space: pre">any/every/whatever_connectec_county = {<br>    max_naval_distance = 500<br>    allow_one_county_land_gap = yes<br>every_connected_county = { limit = { &lt;triggers&gt; } &lt;effects&gt; }</code> | landed title | landed title |
...
| ordered_past_holder | Iterate through all past owners of a title from earliest to latest | <code style="white-space: pre">ordered_any_past_holder = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_past_holder_reversed | Iterate through all past owners of a title from latest to earliest | <code style="white-space: pre">ordered_any_past_holder_reversed = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_claimant | Iterate through all claimants to title. parameters: explicit = yes/no/all - default yes | <code style="white-space: pre">ordered_claimant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_connected_county | Iterate through all counties connected to this one. Is based on top liege | <code style="white-space: pre">any/every/whatever_connectec_county = {<br>    max_naval_distance = 500<br>    allow_one_county_land_gap = yes<br>ordered_connected_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_controlled_faith | Iterate through all faiths headed by a title | <code style="white-space: pre">ordered_controlled_faith = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | faith |
| ordered_county_province | Iterate through all provinces in a county | <code style="white-space: pre">ordered_county_province = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | province |
| ordered_county_struggle | Iterate through all struggles that a county is involved in. | <code style="white-space: pre">ordered_county_struggle = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | struggle |
| ordered_de_jure_county | Iterate through all counties within this dejure title | <code style="white-space: pre">ordered_de_jure_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_de_jure_county_holder | Iterate through all characters directly holding counties within this dejure title | <code style="white-space: pre">ordered_de_jure_county_holder = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_de_jure_top_liege | Iterate through all top lieges of the counts within this dejure title | <code style="white-space: pre">ordered_de_jure_top_liege = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_dejure_vassal_title_holder | Iterate through all the vassal holders of the title | <code style="white-space: pre">ordered_dejure_vassal_title_holder = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_direct_de_facto_vassal_title | Iterate through all de facto vassal titles | <code style="white-space: pre">ordered_direct_de_facto_vassal_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_direct_de_jure_vassal_title | Iterate through the all de jure vassals titles | <code style="white-space: pre">ordered_direct_de_jure_vassal_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_election_candidate | Iterate through all characters who are valid candidates in an election for a title | <code style="white-space: pre">ordered_election_candidate = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_elector | Iterate through all characters who are valid electors in an election for a title | <code style="white-space: pre">ordered_elector = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
...
| ordered_neighboring_county | Iterate through all neighboring counties. Can only be used in county scope | <code style="white-space: pre">ordered_neighboring_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_this_title_or_de_jure_above | Iterate through this title and all its dejure liege titles | <code style="white-space: pre">ordered_this_title_or_de_jure_above = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_heir | Line of succession for the scoped title | <code style="white-space: pre">ordered_title_heir = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | character |
| ordered_title_joined_faction | Iterate through all factions joined the scope landed title | <code style="white-space: pre">ordered_title_joined_faction = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | faction |
| ordered_title_to_title_neighboring_and_across_water_county | Scopes from a title to a neighboring county (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_and_across_water_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_and_across_water_duchy | Scopes from a title to a neighboring duchy (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_and_across_water_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_and_across_water_empire | Scopes from a title to a neighboring empire (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_and_across_water_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_and_across_water_kingdom | Scopes from a title to a neighboring kingdom (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_and_across_water_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_county | Scopes from a title to a neighboring county (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_duchy | Scopes from a title to a neighboring duchy (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_empire | Scopes from a title to a neighboring empire (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
| ordered_title_to_title_neighboring_kingdom | Scopes from a title to a neighboring kingdom (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_title_to_title_neighboring_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | landed title | landed title |
...
| random_connected_county | Iterate through all counties connected to this one. Is based on top liege | <code style="white-space: pre">any/every/whatever_connectec_county = {<br>    max_naval_distance = 500<br>    allow_one_county_land_gap = yes<br>random_connected_county = { limit = { &lt;triggers&gt; } (optional) weight = { mtth } &lt;effects&gt; }</code> | landed title | landed title |
...
| set_county_culture | sets the culture of a county | <code style="white-space: pre">usage:<br>set_county_culture = culture:english/root.character_culture</code> | landed title | culture |
...
| change_cultural_acceptance | Changes cultural acceptance with the target culture | <code style="white-space: pre">change_cultural_acceptance = {<br>target = &lt;culture&gt;<br>value = script value<br>desc = dynamic desc. Description that'll show when hovering over the acceptance tooltip in the culture window<br> }</code> | culture |  |
...
| ordered_culture_county | Iterate through all counties of the culture | <code style="white-space: pre">ordered_culture_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | landed title |
| ordered_culture_duchy | Iterate through all duchies of the culture (duchies with at least one county of the culture | <code style="white-space: pre">ordered_culture_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | landed title |
| ordered_culture_empire | Iterate through all empires of the culture (empires with at least one county of the culture | <code style="white-space: pre">ordered_culture_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | landed title |
| ordered_culture_kingdom | Iterate through all kingdoms of the culture (kingdoms with at least one county of the culture | <code style="white-space: pre">ordered_culture_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | landed title |
| ordered_parent_culture | Iterate through all parent cultures | <code style="white-space: pre">ordered_parent_culture = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | culture |
| ordered_parent_culture_or_above | Iterate through all parent cultures or above | <code style="white-space: pre">ordered_parent_culture_or_above = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | culture | culture |
...
| divide_war_chest | The scoped GHW gives out its war-chest in full or in part. | <code style="white-space: pre">divide_war_chest = {<br>    defenders = yes (default to attackers instead)<br>    faction = script value (default 1 for 100%)<br>    gold = no (default = yes)<br>    piety = no (default = yes)<br>    prestige = no (default = yes)<br>}</code> | great holy war |  |
...
| ordered_pledged_attacker | Iterate through all pledged attackers within a great holy war | <code style="white-space: pre">ordered_pledged_attacker = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | great holy war | character |
| ordered_pledged_defender | Iterate through all pledged defenders within a great holy war | <code style="white-space: pre">ordered_pledged_defender = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | great holy war | character |
...
| ordered_defensive_great_holy_wars | Iterate through all great holy wars this faith is defending against | <code style="white-space: pre">ordered_defensive_great_holy_wars = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | great holy war |
| ordered_faith_character | Iterate through characters of the scoped faith | <code style="white-space: pre">ordered_faith_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | character |
| ordered_faith_holy_order | Iterate through all holy orders of the faith | <code style="white-space: pre">ordered_faith_holy_order = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | holy order |
| ordered_faith_playable_ruler | Iterate through playable rulers of the scoped faith | <code style="white-space: pre">ordered_faith_playable_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | character |
| ordered_faith_ruler | Iterate through rulers of the scoped faith | <code style="white-space: pre">ordered_faith_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | character |
| ordered_holy_site | Iterate through all holy site baronies of a faith | <code style="white-space: pre">ordered_holy_site = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | faith | landed title |
...
| add_province_modifier | Add a modifier to a province | <code style="white-space: pre">add_province_modifier = name<br>add_province_modifier = { modifier = name days/weeks/months/years = int }<br>You can also add an optional 'desc' field. This is a dynamic description that'll be used for your timed modifier</code> | province |  |
...
| ordered_killed_character | Iterate through all kills of a character | <code style="white-space: pre">ordered_killed_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character, artifact | character |
...
| ordered_interloper_ruler | Iterate through all characters that are interloper in a struggle. | <code style="white-space: pre">ordered_interloper_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | struggle | character |
| ordered_involved_ruler | Iterate through all characters that are involved in a struggle. | <code style="white-space: pre">ordered_involved_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | struggle | character |
...
| ordered_scheme_agent | Iterate through all agents in the scheme | <code style="white-space: pre">ordered_scheme_agent = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | scheme | character |
...
| ordered_secret_knower | Iterate through all characters who know the secret | <code style="white-space: pre">ordered_secret_knower = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | secret | character |
| ordered_secret_participant | Iterate through participants in a secret | <code style="white-space: pre">ordered_secret_participant = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | secret | character |
...
| add_dynasty_modifier | Add a modifier to a dynasty | <code style="white-space: pre">add_dynasty_modifier = name<br>add_dynasty_modifier = { modifier = name days/weeks/months/years = int }<br>You can also add an optional 'desc' field. This is a dynamic description that'll be used for your timed modifier</code> | dynasty |  |
...
| ordered_dynasty_member | Iterate through all dynasty members | <code style="white-space: pre">ordered_dynasty_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | dynasty | character |
...
| ordered_leased_title | Iterate through all titles leased to a holy order | <code style="white-space: pre">ordered_leased_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | holy order | landed title |
...
| add_to_global_variable_list | Adds the event target to a variable list | <code style="white-space: pre">add_to_variable_list = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
...
| add_to_local_variable_list | Adds the event target to a variable list | <code style="white-space: pre">add_to_variable_list = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
...
| add_to_variable_list | Adds the event target to a variable list | <code style="white-space: pre">add_to_variable_list = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
...
| change_global_variable | Changes the value or a numeric variable | <code style="white-space: pre">change_variable = { name = X operation = Y }<br>Where X is the name of the numeric variable to modify<br>Where the valid operations are add, subtract, multiply, divide and modulo<br>Where Y is a fixed point value, script value or event target of a value type</code> | none |  |
| change_local_variable | Changes the value or a numeric variable | <code style="white-space: pre">change_variable = { name = X operation = Y }<br>Where X is the name of the numeric variable to modify<br>Where the valid operations are add, subtract, multiply, divide and modulo<br>Where Y is a fixed point value, script value or event target of a value type</code> | none |  |
| change_title_holder | = { | <code style="white-space: pre">holder = 'Character that should get the title'<br>change = 'previously created title_and_vassal_change', adds a title change, will not transfer vassals<br>take_baronies = yes # Optional; if set, will cause baronies to be taken (rather than vassalized) as well if this title is a county<br>government_base = character # Optional, if the character getting the title was unlanded, their new government will be based on the government of government_base. If no government_base is specified, the government will be based on holder's government.</code> | none |  |
...
| change_variable | Changes the value or a numeric variable | <code style="white-space: pre">change_variable = { name = X operation = Y }<br>Where X is the name of the numeric variable to modify<br>Where the valid operations are add, subtract, multiply, divide and modulo<br>Where Y is a fixed point value, script value or event target of a value type</code> | none |  |
| clamp_global_variable | Clamps a variable the specified max and min | <code style="white-space: pre">clamp_variable = { name = X max = Y min = Z }<br>Where X is the name of the variable<br>Where Y and Z are script values</code> | none |  |
| clamp_local_variable | Clamps a variable the specified max and min | <code style="white-space: pre">clamp_variable = { name = X max = Y min = Z }<br>Where X is the name of the variable<br>Where Y and Z are script values</code> | none |  |
| clamp_variable | Clamps a variable the specified max and min | <code style="white-space: pre">clamp_variable = { name = X max = Y min = Z }<br>Where X is the name of the variable<br>Where Y and Z are script values</code> | none |  |
...
| close_view | Tries to close the defined view. | <code style="white-space: pre">Take care to set player if effect is executed not through UI but through synchronized code. Else all the window will open for all players.<br>close_view = {<br>    view = &lt;view name&gt;<br>    player = scope:character # optional, else closes for all players who execute the effect<br>}simplified: close_view = &lt;view name&gt;</code> | none |  |
...
| open_interaction_window | Tries to open the defined view. | <code style="white-space: pre">interaction = interaction_key - the interaction object key to open<br>redirect = [yes\|no] - yes by default, redirect the actor and recipients ( only works if secondary_actor and secondary_recipient are not setup or are invalid)<br>actor = character_actor - must be defined, must coincide with the current player<br>recipient = character_actor - must be defined<br>secondary_actor = character_secontary_actor - optional<br>secondary_recipient = character_secondary_recipient - optional</code> | none |  |
| open_view | Tries to open the defined view without scope data. | <code style="white-space: pre">Take care to set player if effect is executed not through UI but through synchronized code. Else all the window will open for all players.<br>open_view = {<br>    view = &lt;view name&gt;<br>    view_message = &lt;view specific instruction&gt; # optional<br>    player = scope:character # optional, else shows for all players who execute the effect<br>}simplified: open_view = &lt;view name&gt;</code> | none |  |
| open_view_data | Tries to open the defined view with scope data. | <code style="white-space: pre">Take care to set player if effect is executed not through UI but through synchronized code. Else all the window will open for all players.<br>open_view_data = {<br>    view = &lt;view name&gt;<br>    view_message = &lt;view specific instruction&gt; # optional<br>    player = scope:character # optional, else shows for all players who execute the effect<br>}<br>simplified: open_view_data = &lt;view name&gt;<br>for example: scope:faith = { open_view_data = faith_conversion }</code> | none |  |
| ordered_artifact | Iterate through all existing artifacts | <code style="white-space: pre">ordered_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | artifact |
| ordered_barony | Iterate through all baronies in the game | <code style="white-space: pre">ordered_barony = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_character_with_royal_court | Iterate through all characters with a royal court | <code style="white-space: pre">ordered_character_with_royal_court = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_county | Iterate through all counties in the game | <code style="white-space: pre">ordered_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_county_in_region | Iterate through all counties in the region. Put 'region = region_name' inside it | <code style="white-space: pre">ordered_county_in_region = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_culture_global | Iterate through all cultures in the game | <code style="white-space: pre">ordered_culture_global = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | culture |
| ordered_duchy | Iterate through all duchies in the game | <code style="white-space: pre">ordered_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_empire | Iterate through all empires in the game | <code style="white-space: pre">ordered_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_in_global_list | Iterate through all items in global list. list = name or variable = name | <code style="white-space: pre">ordered_in_global_list = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none |  |
| ordered_in_list | Iterate through all items in list. list = name or variable = name | <code style="white-space: pre">ordered_in_list = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none |  |
| ordered_in_local_list | Iterate through all items in local list. list = name or variable = name | <code style="white-space: pre">ordered_in_local_list = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none |  |
| ordered_independent_ruler | Independent rulers list with a COUNT tier or above who hold land | <code style="white-space: pre">ordered_independent_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_inspiration | Iterate through all inspirations in the world | <code style="white-space: pre">ordered_inspiration = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | inspiration |
| ordered_inspired_character | Iterate through all characters with an inspirations in the world | <code style="white-space: pre">ordered_inspired_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_kingdom | Iterate through all kingdoms in the game | <code style="white-space: pre">ordered_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | landed title |
| ordered_living_character | Iterate through all living characters | <code style="white-space: pre">ordered_living_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_player | Iterate through all player characters | <code style="white-space: pre">ordered_player = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_pool_character | Iterate through all characters in the pool of the given province | <code style="white-space: pre">ordered_pool_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
| ordered_province | Iterate through all provinces (skips non-land and impassable provinces) | <code style="white-space: pre">ordered_province = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | province |
| ordered_religion_global | Iterate through all religions in the game | <code style="white-space: pre">ordered_religion_global = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | religion |
| ordered_ruler | Rulers list with a COUNT tier o above | <code style="white-space: pre">ordered_ruler = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | none | character |
...
| remove_list_global_variable | Removes the target from a variable list | <code style="white-space: pre">remove_list_variable = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
| remove_list_local_variable | Removes the target from a variable list | <code style="white-space: pre">remove_list_variable = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
| remove_list_variable | Removes the target from a variable list | <code style="white-space: pre">remove_list_variable = { name = X target = Y }<br>Where X is the name of the variable<br>Where Y is an event target</code> | none |  |
...
| round_global_variable | Rounds a variable to the nearest specified value | <code style="white-space: pre">clamp_variable = { name = X nearest = Y }<br>Where X is the name of the variable<br>Where Y is a script value</code> | none |  |
| round_local_variable | Rounds a variable to the nearest specified value | <code style="white-space: pre">clamp_variable = { name = X nearest = Y }<br>Where X is the name of the variable<br>Where Y is a script value</code> | none |  |
| round_variable | Rounds a variable to the nearest specified value | <code style="white-space: pre">clamp_variable = { name = X nearest = Y }<br>Where X is the name of the variable<br>Where Y is a script value</code> | none |  |
| run_interaction | Execute/send the given interaction | <code style="white-space: pre">interaction = interaction_key - the interaction object key to run<br>redirect = [yes\|no] - yes by default, redirect the actor and recipients ( only works if secondary_actor and secondary_recipient are not setup or are invalid)<br>actor = character_actor - must be defined<br>recipient = character_actor - must be defined<br>secondary_actor = character_secontary_actor - optional<br>secondary_recipient = character_secondary_recipient - optional<br>execute_threshold = accept/maybe/decline - will be executed immediately if the AI response is at least this<br>send_threshold = accept/maybe/decline - will be sent if the AI response is at least this<br>At least one of execute_threshold and send_threshold must be set</code> | none |  |
...
| set_global_variable | Sets a variable | <code style="white-space: pre">set_variable = { name = X value = Y days = Z }<br>Where X is the name of the variable used to then access it<br>Where Y is any event target, bool, value, script value or flag (flag:W)<br>An optional days where Z is the number of days or script value; this can't refer to another variable’s value directly and should instead point to a script value that then checks that variable’s value.<br>This variable will be accessible with &lt;type_&gt;var:X. With type being in a scope object or in a top scope<br>Can also be used as set_variable = X (equivalent to set_variable = { name = X value = yes })</code> | none |  |
| set_local_variable | Sets a variable | <code style="white-space: pre">set_variable = { name = X value = Y days = Z }<br>Where X is the name of the variable used to then access it<br>Where Y is any event target, bool, value, script value or flag (flag:W)<br>An optional days where Z is the number of days or script value; a variable cannot be used in this section, and you should instead use a script value that checks the variable’s value.<br>This variable will be accessible with &lt;type_&gt;var:X. With type being in a scope object or in a top scope<br>Can also be used as set_variable = X (equivalent to set_variable = { name = X value = yes })</code> | none |  |
| set_pregnancy_gender | Set the gender of the unborn child | `set_pregnancy_gender = female/male/random` | none |  |
| set_variable | Sets a variable | <code style="white-space: pre">set_variable = { name = X value = Y days = Z }<br>Where X is the name of the variable used to then access it<br>Where Y is any event target, bool, value, script value or flag (flag:W)<br>An optional days where Z is the number of days or script value; this cannot refer to another variable’s value directly, and should instead refer to a script value that checks that variable’s value.<br>This variable will be accessible with &lt;type_&gt;var:X. With type being in a scope object or in a top scope<br>Can also be used as set_variable = X (equivalent to set_variable = { name = X value = yes })</code> | none |  |
| setup_claim_cb | = { attacker = scope:attacker | <code style="white-space: pre">defender = scope:defender <br> claimant = scope:claimant <br>  change = scope:change victory = yes/no <br> take_occupied = yes/no } , this effect will add a scope:cb_prestige_factor with a value based on what's being taken<br>civil_war = yes # Will vassalize people based on their capital location rather than being fully contained. Won't steal land from people not vassalized</code> | none |  |
| setup_de_jure_cb | = { attacker = scope:attacker | <code style="white-space: pre">defender = scope:defender <br>change = scope:change<br>victory = yes/no<br>title = some title - Optional; will make it target a specific dejure title rather than *everything* that is dejure the attacker's<br>}, this effect will add a scope:cb_prestige_factor with a value based on what's being taken</code> | none |  |
| setup_invasion_cb | = { attacker = scope:attacker | <code style="white-space: pre">defender = scope:defender <br>  change = scope:change victory = yes/no <br> take_occupied = yes/no } , this effect will add a scope:cb_prestige_factor with a value based on what's being taken</code> | none |  |
...
| trigger_event | triggers an event or on_action | <code style="white-space: pre">trigger_event = { id = X days/months/years = Y delayed = yes/no } (for events)<br>or<br>trigger_event = { on_action = X days/months/years = Y delayed = yes/no } (for on_actions)<br>Days/months/years are optional and equal to 0 if not specified. If specified, Y can be a value or an inclusive interval "{ A B }" from which the duration will be picked Scopes._Randomly.<br>delayed = yes will delay the event/on_action to the next event tick even when there's no explicit time interval set (delay, but fire ASAP). By default, delayed = no</code> | none |  |
| try_create_important_action | Tries to create an important action notification. Will keep the current one if already exists. | <code style="white-space: pre">important_action_type = important_action_type_key - the important action object key to create. Must be defined.<br>actor = character_actor - optional, can be used by the important action effect<br>recipient = character_actor - optional, can be used by the important action effect<br>secondary_actor = character_secontary_actor - optional, can be used by the important action effect<br>secondary_recipient = character_secondary_recipient - optional, can be used by the important action effect<br>landed_title = landed_title - optional, can be used by the important action effect<br>war = war - optional, can be used by the important action effect<br>artifact = artifact - optional, can be used by the important action effect<br>culture = culture - optional, can be used by the important action effect</code> | none |  |
| try_create_suggestion | Tries to create an suggestuib notification. Will keep the current one if already exists. | <code style="white-space: pre">suggestion_type = suggestion_type_key - the suggestion action object key to create. Must be defined.<br>actor = character_actor - optional, can be used by the suggestion effect<br>recipient = character_actor - optional, can be used by the suggestion effect<br>secondary_actor = character_secontary_actor - optional, can be used by the suggestion effect<br>secondary_recipient = character_secondary_recipient - optional, can be used by the suggestion effect<br>landed_title = landed_title - optional, can be used by the suggestion effect</code> | none |  |
...
| add_character_flag | adds a character flag | <code style="white-space: pre">usage:<br>add_character_flag = X<br>add_character_flag = { flag = X days/weeks/years = Y }<br>where X is the name of the flag and Y is a value or value interval "{ min max }"</code> | character |  |
| add_character_modifier | Add a modifier to a character | <code style="white-space: pre">add_character_modifier = name<br>add_character_modifier = { modifier = name days/weeks/months/years = int }<br>You can also add an optional 'desc' field. This is a dynamic description that'll be used for your timed modifier</code> | character |  |
...
| add_hook | Adds a hook on a character | <code style="white-space: pre">add_hook = { type = X, target = Y, secret = Z, days/months/years = W  }<br>Note: days/months/years optional (taken from hook type otherwise) and can be a value or an interval, secret required for hook types that require it.<br>Does send a toast to the player if it's involved.</code> | character |  |
| add_hook_no_toast | Adds a hook on a character | <code style="white-space: pre">add_hook = { type = X, target = Y, secret = Z, days/months/years = W  }<br>Note: days/months/years optional (taken from hook type otherwise) and can be a value or an interval, secret required for hook types that require it.<br>Does NOT send a toast to the player.</code> | character |  |
...
| add_relation_flag | Adds a flag to an existing relation | <code style="white-space: pre">add_relation_flag = {<br>  relation = scripted_relation<br>  flag = flag_name (declared in the relation's script)<br>  target = other_character<br>}</code> | character |  |
...
| add_truce_both_ways | Sets the both-way truce against the specified character | <code style="white-space: pre">'character' specifies the target character<br>'override' says whether it should replace the previous truce even if shorter<br>'years / months / days' sets the duration of the truce<br>'result' specifies the result from the scope character's point of view ('white_peace' by default)<br>'casus_belli' sets the casus belli scope that caused the truce, mutually exclusive with 'name'<br>'name' sets a custom description. Dynamic description with the current scope<br>'war' sets the war that caused the truce, mutually exclusive with 'casus_belli'<br>add_truce_both_ways = { character = X years/months/days = Y override = yes/no result = victory/defeat/white_peace casus_belli/war = Z }</code> | character |  |
| add_truce_one_way | Sets the truce against the specified character | <code style="white-space: pre">'character' specifies the target character<br>'override' says whether it should replace the previous truce even if shorter<br>'years / months / days' sets the duration of the truce<br>'result' specifies the result from the scope character's point of view ('white_peace' by default)<br>'casus_belli' sets the casus belli scope that caused the truce, mutually exclusive with 'name'<br>'name' sets a custom description. Dynamic description with the current scope<br>add_truce_one_way = { character = X years/months/days = Y override = yes/no result = victory/defeat/white_peace casus_belli/war = Z }</code> | character |  |
...
| appoint_court_position | Appoints the target character in target court position within scoped character's court | <code style="white-space: pre">recipient = character scope - target character to receive the title<br>court_position = court position type - court position type to assign the receiver</code> | character |  |
| assign_council_task | Assigns the target character to the council task | <code style="white-space: pre">assign_council_task = {<br>    council_task = council_task_scope<br>    target = character_taking_the_position    fire_on_actions = [yes]<br>}</code> | character |  |
...
| change_first_name | Change the first name of a character | <code style="white-space: pre">change_first_name = &lt;localization_key&gt;<br>change_first_name = scope:name/var:name # containing a flag with a localization key<br>change_first_name = { template_character = scope:character } # copy name from the template character</code> | character |  |
...
| create_inspiration | create_inspiration = inspiration_type | <code style="white-space: pre">Creates an inspiration of a given type owned by scoped character, new inspiration is saved as scope:new_inspiration<br>create_inspiration = { type = inspiration_type gold = script_value }<br>As above but overrides the base cost compared to the one defined in the type</code> | character |  |
| create_story | creates and initializes a story cycle with the current character as owner | <code style="white-space: pre">create_story = story_type<br>create_story = {<br>    type = story_type<br>    save_scope_as/save_temporary_scope_as = scope_name # optional way to get a reference to the new story}</code> | character |  |
...
| every_character_struggle | Iterate through all struggles that character is involved in. | <code style="white-space: pre">every_character_struggle = { limit = { &lt;triggers&gt; } &lt;effects&gt; }<br>Optional: Narrow down the involvement status *_character_struggle = { involvement = involved | interloper }</code> | character | struggle |
...
| every_opposite_sex_spouse_candidate | Iterate through all the spouse candidates of the opposite sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>every_opposite_sex_spouse_candidate = { limit = { &lt;triggers&gt; } &lt;effects&gt; }</code> | character | character |
...
| every_same_sex_spouse_candidate | Iterate through all the spouse candidates of the same sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>every_same_sex_spouse_candidate = { limit = { &lt;triggers&gt; } &lt;effects&gt; }</code> | character | character |
...
| every_spouse_candidate | Iterate through all the spouse candidates of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>every_spouse_candidate = { limit = { &lt;triggers&gt; } &lt;effects&gt; }</code> | character | character |
...
| force_add_to_scheme | Adds a character as an agent to the scheme and forces them to stay | <code style="white-space: pre">{<br>    scheme = target_Scheme<br>    days/months/years = duration<br>}</code> | character |  |
| force_character_skill_recalculation | Forces a character's skills to be recalculated immediately, bypassing the wait for the daily tick. | <code style="white-space: pre">NOTE: Only use this when *absolutely* necessary, as it will impact performance negatively if misused<br>   Usage: force_character_skill_recalculation = yes/no</code> | character |  |
...
| join_faction_forced | the character in the scope is forced to join a faction by a character for a defined time, | <code style="white-space: pre">join_faction_forced = {<br>    faction = X<br>    forced_by = Y<br>    days/months/years = duration<br>}</code> | character |  |
...
| make_pregnant | makes a character pregnant | <code style="white-space: pre">father= 'the real father'<br> number_of_children= X<br> known_bastard=yes/no</code> | character |  |
| make_pregnant_no_checks | makes a character pregnant. Doesn't error on things like celibacy | <code style="white-space: pre">father= 'the real father'<br> number_of_children= X<br> known_bastard=yes/no</code> | character |  |
...
| ordered_alert_creatable_title | Iterate through all titles that can be created by the character. (only for alerts) | <code style="white-space: pre">ordered_alert_creatable_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_alert_usurpable_title | Iterate through all titles that can be usurped by the character. (only for alerts) | <code style="white-space: pre">ordered_alert_usurpable_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_ally | Iterate through all allies | <code style="white-space: pre">ordered_ally = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_ancestor | Iterate through all the ancestors of the scope character up to 5 generations | <code style="white-space: pre">ordered_ancestor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_army | Iterate through all armies | <code style="white-space: pre">ordered_army = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | army |
| ordered_character_artifact | Iterate through all artifacts in a given characters inventory | <code style="white-space: pre">ordered_character_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | artifact |
| ordered_character_struggle | Iterate through all struggles that character is involved in. | <code style="white-space: pre">ordered_character_struggle = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }<br>Optional: Narrow down the involvement status *_character_struggle = { involvement = involved | interloper }</code> | character | struggle |
| ordered_character_to_title_neighboring_and_across_water_county | Scopes from a character to a neighboring county (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_and_across_water_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_and_across_water_duchy | Scopes from a character to a neighboring duchy (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_and_across_water_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_and_across_water_empire | Scopes from a character to a neighboring empire (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_and_across_water_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_and_across_water_kingdom | Scopes from a character to a neighboring kingdom (incl. across water, looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_and_across_water_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_county | Scopes from a character to a neighboring county (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_duchy | Scopes from a character to a neighboring duchy (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_empire | Scopes from a character to a neighboring empire (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_to_title_neighboring_kingdom | Scopes from a character to a neighboring kingdom (looking trough the de Jure lieges) | <code style="white-space: pre">ordered_character_to_title_neighboring_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_character_war | Wars of the scoped character | <code style="white-space: pre">ordered_character_war = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | war |
| ordered_child | Iterate through all children | <code style="white-space: pre">ordered_child = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_claim | Iterate through the titles of all claims held by a character; parameters: explicit = yes/no/all pressed = yes/no/all | <code style="white-space: pre">ordered_claim = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_claimed_artifact | Iterate through all claimed artifacts of the scoped character | <code style="white-space: pre">ordered_claimed_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | artifact |
| ordered_close_family_member | Iterate through all the close family [father, mother, siblings, children, grandparents] | <code style="white-space: pre">ordered_close_family_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_close_or_extended_family_member | Iterate through all the close and extended relatives [father, mother, siblings, children, grandparents, uncles/aunts, nephew/niece, cousins] | <code style="white-space: pre">ordered_close_or_extended_family_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_concubine | Iterate through all concubines | <code style="white-space: pre">ordered_concubine = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_consort | Iterate through all consorts (concubines and spouses) | <code style="white-space: pre">ordered_consort = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_councillor | Iterate through all councillors | <code style="white-space: pre">ordered_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_court_position_employer | Iterates through all characters that employ the scoped character in any court position. | <code style="white-space: pre">ordered_court_position_employer = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_court_position_holder | Iterates through all characters employed by the scoped character in the target court position. | <code style="white-space: pre">ordered_court_position_holder = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_courtier | Iterate through all courtiers | <code style="white-space: pre">ordered_courtier = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_courtier_away | Iterate through all courtiers that are away | <code style="white-space: pre">ordered_courtier_away = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_courtier_or_guest | Iterate through all courtiers and guests (pool and foreign court guests) | <code style="white-space: pre">ordered_courtier_or_guest = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_de_jure_claim | Iterate through all de jure claims for a character | <code style="white-space: pre">ordered_de_jure_claim = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_diplomacy_councillor | Iterate through all diplomacy-based councillors | <code style="white-space: pre">ordered_diplomacy_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_directly_owned_province | Iterate through all directly owned provinces | <code style="white-space: pre">ordered_directly_owned_province = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | province |
| ordered_election_title | Iterate through all titles the scoped character can vote on | <code style="white-space: pre">ordered_election_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_equipped_character_artifact | Iterate through all equipped artifacts in a given characters inventory | <code style="white-space: pre">ordered_equipped_character_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | artifact |
| ordered_extended_family_member | Iterate through all the extended family [uncles/aunts, nephew/niece, cousins] | <code style="white-space: pre">ordered_extended_family_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_foreign_court_guest | Iterate through all guests visiting from another court (in contrast to pool_guest they have a liege) | <code style="white-space: pre">ordered_foreign_court_guest = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_former_concubine | Iterate through all former concubines. Not persisted past death | <code style="white-space: pre">ordered_former_concubine = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_former_concubinist | Iterate through all former concubinists. Not persisted past death | <code style="white-space: pre">ordered_former_concubinist = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_former_spouse | Iterate through all former spouses | <code style="white-space: pre">ordered_former_spouse = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_general_councillor | Iterate through all councillors that are not related to a skill | <code style="white-space: pre">ordered_general_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_heir | Heirs of the scoped character | <code style="white-space: pre">ordered_heir = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_heir_title | Iterate through all landed titles character is heir to | <code style="white-space: pre">ordered_heir_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_heir_to_title | Iterate through all titles the scoped character is heir to | <code style="white-space: pre">ordered_heir_to_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_held_title | Iterate through all held landed titles | <code style="white-space: pre">ordered_held_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_hired_mercenary | Iterate through all hired mercenary companies | <code style="white-space: pre">ordered_hired_mercenary = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | mercenary company |
| ordered_hooked_character | Iterate through all characters this character has a hook on | <code style="white-space: pre">ordered_hooked_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_hostile_raider | Iterate through anyone the character is hostile to due to their top-liege's realm having been raided | <code style="white-space: pre">ordered_hostile_raider = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_intrigue_councillor | Iterate through all intrigue-based councillors | <code style="white-space: pre">ordered_intrigue_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_knight | Iterate through all knights | <code style="white-space: pre">ordered_knight = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_known_secret | Iterate through all secrets known by the character | <code style="white-space: pre">ordered_known_secret = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | secret |
| ordered_learning_councillor | Iterate through all learning-based councillors | <code style="white-space: pre">ordered_learning_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_liege_or_above | Iterate through all lieges above a character (skipping the character themselves) | <code style="white-space: pre">ordered_liege_or_above = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_martial_councillor | Iterate through all martial-based councillors | <code style="white-space: pre">ordered_martial_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_neighboring_and_across_water_realm_same_rank_owner | A sub-realm or realm bordering the scope character's realm (including across water) and has the same rank as the scope character (look for lieges of he owner of the land if necessary) | <code style="white-space: pre">ordered_neighboring_and_across_water_realm_same_rank_owner = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_neighboring_and_across_water_top_liege_realm | A realm with a different top liege neighboring the realm of the scope character's top liege (including across water); switches to the realm's top title. Can be based on borders a day or two out of date | <code style="white-space: pre">ordered_neighboring_and_across_water_top_liege_realm = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_neighboring_and_across_water_top_liege_realm_owner | A realm with a different top liege neighboring the realm of the scope character's top liege (including across water); switches to the holder of the realm. Can be based on borders a day or two out of date | <code style="white-space: pre">ordered_neighboring_and_across_water_top_liege_realm_owner = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_neighboring_realm_same_rank_owner | A sub-realm or realm bordering the scope character's realm and has the same rank as the scope character (look for lieges of he owner of the land if necessary) | <code style="white-space: pre">ordered_neighboring_realm_same_rank_owner = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_neighboring_top_liege_realm | A realm with a different top liege neighboring the realm of the scope character's top liege; switches to the realm's top title. Can be based on borders a day or two out of date | <code style="white-space: pre">ordered_neighboring_top_liege_realm = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_neighboring_top_liege_realm_owner | A realm with a different top liege neighboring the realm of the scope character's top liege; switches to the holder of the realm. Can be based on borders a day or two out of date | <code style="white-space: pre">ordered_neighboring_top_liege_realm_owner = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_opposite_sex_spouse_candidate | Iterate through all the spouse candidates of the opposite sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>ordered_opposite_sex_spouse_candidate = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_owned_story | Iterate through all owned stories for a character | <code style="white-space: pre">ordered_owned_story = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | story cycle |
| ordered_parent | Iterate through all (both) parents | <code style="white-space: pre">ordered_parent = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_patroned_holy_order | Iterate through all holy orders that the scoped character is a patron of | <code style="white-space: pre">ordered_patroned_holy_order = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | holy order |
| ordered_personal_claimed_artifact | Iterate through all personally claimed artifacts of the scoped character | <code style="white-space: pre">ordered_personal_claimed_artifact = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | artifact |
| ordered_pinned_character | Iterate through characters this player has pinned | <code style="white-space: pre">ordered_pinned_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_pinning_character | Iterate through characters whose player has this character pinned | <code style="white-space: pre">ordered_pinning_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_played_character | Iterate through all characters the player playing this character has played. Matches the game over legacy, except for excluding the currently played character | <code style="white-space: pre">ordered_played_character = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_player_heir | Iterate through player heirs, capped at the first 10 | <code style="white-space: pre">ordered_player_heir = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_pool_guest | Iterate through all guests visiting the court from the pool (in contrast to foreign_court_guest they don't have a liege) | <code style="white-space: pre">ordered_pool_guest = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_potential_marriage_option | Iterate through all potential selectable marriage or betrohed options | <code style="white-space: pre">ordered_potential_marriage_option = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_pretender_title | Iterate through all landed titles character is pretender to | <code style="white-space: pre">ordered_pretender_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_primary_war_enemy | Iterate through all primary war enemies | <code style="white-space: pre">ordered_primary_war_enemy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_prisoner | Iterate through all prisoners | <code style="white-space: pre">ordered_prisoner = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_prowess_councillor | Iterate through all prowess-based councillors | <code style="white-space: pre">ordered_prowess_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_raid_target | Iterate through anyone the character is hostile to due to having raided them. Only returns top lieges | <code style="white-space: pre">ordered_raid_target = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_realm_county | Iterate through all counties in the realm. Based on top liege | <code style="white-space: pre">ordered_realm_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_realm_de_jure_duchy | Iterate through all de jure duchies that have at least one county in the realm. Based on top liege | <code style="white-space: pre">ordered_realm_de_jure_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_realm_de_jure_empire | Iterate through all de jure empire that have at least one county in the realm. Based on top liege | <code style="white-space: pre">ordered_realm_de_jure_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_realm_de_jure_kingdom | Iterate through all de jure kingdom that have at least one county in the realm. Based on top liege | <code style="white-space: pre">ordered_realm_de_jure_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_realm_province | Iterate through all realm provinces of a character | <code style="white-space: pre">ordered_realm_province = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | province |
| ordered_relation | Iterate through scripted relations of a given type or multiple types, if someone is multiple relations they will only be in the list once | <code style="white-space: pre">ordered_relation = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_same_sex_spouse_candidate | Iterate through all the spouse candidates of the same sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>ordered_same_sex_spouse_candidate = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_scheme | Iterate through all schemes owned by the character | <code style="white-space: pre">ordered_scheme = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | scheme |
| ordered_secret | Iterate through all secrets of the character | <code style="white-space: pre">ordered_secret = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | secret |
| ordered_sibling | Iterate through all siblings | <code style="white-space: pre">ordered_sibling = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_sponsored_inspiration | Iterate through all sponsored inspirations | <code style="white-space: pre">ordered_sponsored_inspiration = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | inspiration |
| ordered_spouse | Iterate through all spouses | <code style="white-space: pre">ordered_spouse = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_spouse_candidate | Iterate through all the spouse candidates of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>ordered_spouse_candidate = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_stewardship_councillor | Iterate through all stewardship-based councillors | <code style="white-space: pre">ordered_stewardship_councillor = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_sub_realm_barony | Iterate through all baronies in sub-realm | <code style="white-space: pre">ordered_sub_realm_barony = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_sub_realm_county | Iterate through all counties in sub-realm | <code style="white-space: pre">ordered_sub_realm_county = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_sub_realm_duchy | Iterate through all duchies in sub-realm | <code style="white-space: pre">ordered_sub_realm_duchy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_sub_realm_empire | Iterate through all empires in sub-realm | <code style="white-space: pre">ordered_sub_realm_empire = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_sub_realm_kingdom | Iterate through all kingdoms in sub-realm | <code style="white-space: pre">ordered_sub_realm_kingdom = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_sub_realm_title | Iterate through all titles in sub-realm | <code style="white-space: pre">ordered_sub_realm_title = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | landed title |
| ordered_targeting_faction | Iterate through all factions targeting the scope character | <code style="white-space: pre">ordered_targeting_faction = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | faction |
| ordered_targeting_scheme | Iterate through all schemes targeting the character | <code style="white-space: pre">ordered_targeting_scheme = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | scheme |
| ordered_targeting_secret | Iterate through all secrets that target the specified scope | <code style="white-space: pre">ordered_targeting_secret = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | secret |
| ordered_traveling_family_member | Iterate though all characters that should travel with the scoped one (when moving between courts for instance); includes the scoped character | <code style="white-space: pre">ordered_traveling_family_member = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_truce_holder | Iterate through all characters that have a truce on this character | <code style="white-space: pre">ordered_truce_holder = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_truce_target | Iterate through all characters this character has a truce on | <code style="white-space: pre">ordered_truce_target = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_unspent_known_secret | Iterate through all unspent secrets known by the character | <code style="white-space: pre">ordered_unspent_known_secret = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | secret |
| ordered_vassal | Iterate through all DIRECT vassals | <code style="white-space: pre">ordered_vassal = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_vassal_or_below | Iterate through ALL vassals, not just direct vassals | <code style="white-space: pre">ordered_vassal_or_below = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_war_ally | Iterate through all direct war allies | <code style="white-space: pre">ordered_war_ally = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
| ordered_war_enemy | Iterate through all direct war enemies | <code style="white-space: pre">ordered_war_enemy = {<br>limit = { &lt;triggers&gt; }<br>order_by = script_value<br>position = int<br>min = int<br>max = script_value<br>check_range_bounds = no # If you don't want an error logged if the list is smaller than the min/max<br>&lt;effects&gt; }</code> | character | character |
...
| random_character_struggle | Iterate through all struggles that character is involved in. | <code style="white-space: pre">random_character_struggle = { limit = { &lt;triggers&gt; } (optional) weight = { mtth } &lt;effects&gt; }<br>Optional: Narrow down the involvement status *_character_struggle = { involvement = involved | interloper }</code> | character | struggle |
...
| random_opposite_sex_spouse_candidate | Iterate through all the spouse candidates of the opposite sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>random_opposite_sex_spouse_candidate = { limit = { &lt;triggers&gt; } (optional) weight = { mtth } &lt;effects&gt; }</code> | character | character |
...
| random_same_sex_spouse_candidate | Iterate through all the spouse candidates of the same sex of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>random_same_sex_spouse_candidate = { limit = { &lt;triggers&gt; } (optional) weight = { mtth } &lt;effects&gt; }</code> | character | character |
...
| random_spouse_candidate | Iterate through all the spouse candidates of a character. | <code style="white-space: pre">WARNING: THIS IS VERY SLOW DO NOT DO IT OFTEN.<br>random_spouse_candidate = { limit = { &lt;triggers&gt; } (optional) weight = { mtth } &lt;effects&gt; }</code> | character | character |
...
| replace_court_position | Replaces the target character holding target court position within scoped character's court with target character | <code style="white-space: pre">recipient = character scope - target character to receive the title<br>holder = character scope - target character to revoke the target court position of in favour of recipient<br>court_position = court position type - court position type to assign the receiver</code> | character |  |
...
| vassal_contract_set_obligation_level | change the obligation level of the scoped character's vassal contract | <code style="white-space: pre">vassal_contract_set_obligation_level = { type = name level = 1 } # index to obligation level<br>vassal_contract_set_obligation_level = { type = name level = feudal_obligation_low }</code> | character |  |
